...
(c) &emsp; the equation of the plane which passes through $\mathbf{a}$, $\mathbf{b}$ and $\mathbf{c}$;
...
Using the {prf:ref}`geometric definition of a dot product<dot-product-definition>` derive the expression for calculating the shortest distance between a point and a plane. Use your expression to find the shortest distance from the point with position vector $(2, 4, -3)^\mathsf{T}$ to the plane $6x - y - 4z = 3$.
````
